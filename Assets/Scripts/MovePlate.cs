...
public class MovePlate : MonoBehaviour
{
    ...
    refrence.GetComponent<Chesspieces>().GetYBoard());
        ...
        controller.GetComponent<Game>().Setattackwhitenull();
            ...
            controller.GetComponent<Game>().SetMovesNull();

            controller.GetComponent<Game>().SetPawnMovesNull();

        //game score
            controller.GetComponent<Game>().SetScoreNull();
        ...
        for (int i = 0; i < 8; i++)
            for (int j = 0; j < 8; j++)
            {
                if (controller.GetComponent<Game>().GetPosition(i, j) != null)
                {
                    ...
                    controller.GetComponent<Game>().SetScore(controller.GetComponent<Game>().GetPosition(i, j).name);
    ...
    public bool Stalemate(int x, int y)
    {
        controller = GameObject.FindGameObjectWithTag("GameController");
        Game sc = controller.GetComponent<Game>();
        int pieces = 0;
        ...
        if (pieces == 2)
            return true;
        if (SurroundCheck(x, y) && sc.GetMoves() == 0)
            return true;

        if (sc.GetPawnMoves() != 0)
            sc.SetTurnPawn(sc.GetTurns());


        if (sc.GetTurns() - sc.GetTurnPawn() == 50)
            return true;
        ...
        if (sc.PositionOnBoard(x, y))
        {
            ...
            else
            {
                if (ok)
                {
                    ...
                    controller.GetComponent<Game>().Setattackblackking(x, y);
            }
        }
    }
